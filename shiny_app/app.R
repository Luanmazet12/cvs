      ...
      tabItem(tabName = "upload",
        fluidRow(
          box(
            ...
            fileInput("file_upload", "Choisir un ou plusieurs fichiers CSV",
                      accept = ".csv",
                      multiple = TRUE,
                      buttonLabel = "Parcourir",
                      placeholder = "Aucun fichier sélectionné"),
            helpText("Format attendu : exports OpenField — séparateur « ; », décimale « , » — un fichier par athlète")
          )
        ),
        fluidRow(
  ...
  observeEvent(input$file_upload, {
    ...
    tryCatch({
      ...
      dfs <- lapply(seq_len(nrow(files)), function(i) {
        read_openfield_csv(files$datapath[i])
      })
      df <- bind_rows(dfs)
      raw_data(df)
      clean_data(df)
      ...
      n_athletes <- length(unique(df$Player))
      showNotification(
        paste0("\u2705 ", nrow(files), " fichier(s) chargé(s) — ", n_athletes, " athlète(s) détecté(s)"),
        type = "message"
      )
    }, error = function(e) {
